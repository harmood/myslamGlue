...
usage() {
  ...
  echo "  -h, --help    显示本帮助"
  echo
  echo "默认启动：迷宫世界 + 小车 + ROS 话题桥接 + 键盘遥控 + GUI 状态窗口。"
...
for arg in "$@"; do
  case "$arg" in
    ...
    --no-teleop) TELEOP=0 ;;
    --no-gui) GUI=0 ;;
    --world-only) LAUNCH_PKG="maze_world"; LAUNCH_FILE="maze.launch.py"; TELEOP=0; GUI=0 ;;
    -h|--help) usage; exit 0 ;;
    *) LAUNCH_ARGS+=("$arg") ;;
  esac
done
...
if [ "$BUILD" -eq 1 ] || [ ! -f install/setup.bash ]; then
  echo "==> colcon build --packages-select maze_world maze_bot maze_teleop maze_gui --symlink-install"
